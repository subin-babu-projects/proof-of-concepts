package com.example.drive_rest_app;
...
//                            Log.e(TAG, "Couldn't create file.", exception));
            ...
            mDriveServiceHelper.queryFolder("drive_rest_app").addOnSuccessListener(folderList -> {
                ...
                if(folderList.getFiles().size() == 0) {
                    mDriveServiceHelper.createFolder("drive_rest_app").addOnSuccessListener(folderId -> {
                        mDriveServiceHelper.queryFiles("test.txt", folderId).addOnSuccessListener(fileList -> {
                            if (fileList.getFiles().size() == 0) {
                                mDriveServiceHelper.createFile("test.txt", folderId)
                                        .addOnSuccessListener(fileId -> Log.e(TAG, "file create with ID : " + fileId))
                    ...
                    mDriveServiceHelper.queryFiles("test.txt",folderList.getFiles().get(0).getId()).addOnSuccessListener(fileList -> {
                        if(fileList.getFiles().size() == 0){
                            mDriveServiceHelper.createFile("test.txt",folderList.getFiles().get(0).getId())
                                    .addOnSuccessListener(fileId -> Log.e(TAG,"file create with ID : "+ fileId))
                                    .addOnFailureListener(exception ->  Log.e(TAG, "Couldn't create file.", exception));
                        }
                    }).addOnFailureListener(exception -> Log.e(TAG, "Couldn't Count file.", exception));
                }
            });
        }
    }
