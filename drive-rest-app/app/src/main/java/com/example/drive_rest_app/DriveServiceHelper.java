...
public class DriveServiceHelper {
    ...
    public Task<FileList> queryFiles(String fileName,String folderID){
        return Tasks.call(mExecutor, new Callable<FileList>() {
            @Override
            public FileList call() throws Exception {
                return mDriveService.files().list().setQ(String.format("name = '%s' and parents in '%s' and trashed = false",fileName, folderID)).execute();
            }
        });
    }

    public Task<FileList> queryFolder(String folderName){
        return Tasks.call(mExecutor, new Callable<FileList>() {
            @Override
            public FileList call() throws Exception {
                return mDriveService.files().list().setSpaces("drive").setQ("mimeType='application/vnd.google-apps.folder'").execute();
            }
        });
    }
